# references:
# https://github.com/opencv/opencv/blob/4.x/modules/calib3d/src/p3p.cpp
# https://github.com/Kaminyou/P3P-Python-Implement/blob/master/p3p.py
...
class P3P(object):
    ...
    def solve_P4P(self, pixel_points, world_points):
        ...
        mu0, mv0, mu1, mv1, mu2, mv2, mu3, mv3 = pixel_points.flatten()
        mu0 = self.inv_fx * mu0 - self.cx_fx
        mv0 = self.inv_fy * mv0 - self.cy_fy
        norm = math.sqrt(mu0 * mu0 + mv0 * mv0 + 1)
        ...
        mu3 = self.inv_fx * mu3 - self.cx_fx
        mv3 = self.inv_fy * mv3 - self.cy_fy
        ...
        lengths = self.solve_for_lengths(distances, cosines) # 4x3
        if len(lengths) == 0:
            return []
        # print(lengths)
        
        # print("hahahhah")
        undistorted_points = np.array([[mu0,mv0,mk0],[mu1,mv1,mk2],[mu2,mv2,mk2]])
        # print(mu0,mv0,mk0,mu1,mv1,mk1,mu2,mv2,mk2)
        ...
        rvec = rotation_c.as_rotvec().reshape(3,1)
        tvec = best_Ts.reshape(3,1)

        return [rvec, tvec]
    

    def jacobi_4x4(self,A):
        """
        input 
    ...
    def align(self, M_end, X0, Y0, Z0, X1, Y1, Z1, X2, Y2, Z2):
        """
        output
            R: rotation matrix
            T: transform matrix
    ...
    def Quartic_solver(self, parameters):
        ...
        # print("real roots: ", out)
        return out

    def solve_for_lengths(self, distances, cosines):
        """
        input 
            distances: a 3 dim vector with float type
            cosines: a 3 dim vector with float type
        output
            lengths: ideally a 4*3 array but may become n*3, where n = number of real roots
        """
        p, q, r = cosines * 2
        ...
        # print("real_roots: ", len(real_roots))
