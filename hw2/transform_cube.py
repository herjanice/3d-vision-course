...
# load camera pose
camerapose_df = pd.read_pickle("./camera_position.pkl")
for i in range(len(camerapose_df)):
    tmp = camerapose_df.iloc[i]
    load_camera(vis,tmp)
...
vis.run()
# vis.capture_screen_image("p3p_bad_pointcloud.jpg")
vis.destroy_window()
